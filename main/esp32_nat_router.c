                                ...
                                int32_t event_id, void* event_data)
{
    ...
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED)
    {
        ...
        else xEventGroupSetBits(wifi_event_group, WIFI_CONNECTED_BIT);
    }
...
//    ESP_LOGI(TAG, "DNS IP:" IPSTR, IP2STR(&dnsinfo.ip.u_addr.ip4));
    ...
    ESP_LOGI(TAG, "wifi_init returned.");
...
void mainstart()
{
    ...
    if (strcmp(lock, "0") ==0) {
        ESP_LOGI(TAG,"Wait with web server for connection");
        while((strlen(ssid) >0) && !ap_connect && (connect_attempts <= 5)) vTaskDelay(50 / portTICK_PERIOD_MS);
